...
class HowdiesBot:
    ...
    def on_open(self, ws):
        ...
        # NO login payload! Token URL already authenticates
        # Join active rooms
    ...
    def on_close(self, ws, close_status_code, close_msg):
        self.log(f"WebSocket Closed ({close_status_code}). Reconnecting in 5s...")
        if self.running:
            time.sleep(5)
    ...
    def handle_chat(self, data):
        ...
        if text.startswith("!"):
            ...
            if active_game:
                cmd = text.strip()
                self.plugins.handle_command(cmd, room, user, [], avatar_url=avatar_url)
    ...
    def join_room(self, room_id, password="", from_ui=False):
        """
        Join a room.
        from_ui=True -> Room was joined via UI button, 
        don't send join payload to server again.
        """
