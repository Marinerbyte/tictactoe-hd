...
import uuid
import requests
import traceback  # Error dekhne ke liye add kiya
...
class HowdiesBot:
    def __init__(self):
        ...
        # Thread Lock add kiya taki UI aur Bot ladai na kare data ke liye
    ...
    def on_message(self, ws, message):
        try:
            ...
            # Ye line data ko Admin plugin tak bhejegi bina UI ko disturb kiye
            ...
            if handler == "chatroommessage":
                ...
                # Chat commands process karna
                self.plugins.process_message(data)
            
            # 2. User Join (Update UI)
            ...
            # 4. Join Success (Setup Room Data)
                ...
                # Admin ke liye Users ki list maang lo
                self.send_json({"handler": "getusers", "id": uuid.uuid4().hex, "roomid": room_id})
            
            # 5. User List (Update UI)
            elif handler in ["activeoccupants", "userslist"] and room_name in self.room_details:
                users = [u.get('username') for u in data.get("users", [])]
                with self.lock:
                    self.room_details[room_name]['users'] = users
                self.log(f"EVENT: User list updated for '{room_name}'.")
