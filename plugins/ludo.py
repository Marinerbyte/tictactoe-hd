...
# --- THEMES ---
THEMES = {
    'R': {'name': 'Red', 'hex': '#FF3333'},
    'G': {'name': 'Green', 'hex': '#00FF00'},
    'Y': {'name': 'Yellow', 'hex': '#FFFF00'},
    'B': {'name': 'Blue', 'hex': '#0099FF'}
}

# --- TRACK MAPPING (52 Steps) ---
...
def get_xy(step, color, sz, mx, my):
    try:
        ...
        return mx + 7.5*sz, my + 7.5*sz # Fallback to center
...
def draw_board(players, dice=None, rolling=False):
    ...
    for (px, py), p_list in pos_map.items():
        # Offset if multiple players on same spot
        shift_x = -10 if len(p_list) > 1 else 0
    ...
    # 4. Dice
    if rolling:
        utils.write_text(d, (W//2, H//2), "ROLLING...", 40, "white", "center")
    elif dice:
...
def task_update(bot, rid, g, text="Update"):
    try:
        ...
        else: bot.send_message(rid, f"✅ {text} (Image Failed)")
    except: pass
...
def task_roll(bot, rid, g, uid, name, dice):
    try:
        ...
        if f_link:
            ...
            bot.send_message(rid, f"🎲 **{name}** rolled {dice}. (Board Hidden due to Error)")
            
        if not f_link: bot.send_message(rid, f"🎲 Status: {name} rolled {dice}. {msg}")
        else: pass # Already sent image
    ...
    if c == "roll":
        with game_lock:
            ...
            if str(uid) != g.turn[g.idx]: 
                ...
                return True
            # Boost 6 Chance (30%)
            dice = 6 if random.random() < 0.3 else random.randint(1, 5)
