...
def setup(bot):
    print("[Economy] DB-Synced Ledger v6.0 Loaded.")
    ...
    try:
        if is_admin:
            ...
            if cmd == "wipedb" and args and args[0]=="confirm":
                db.execute_query("UPDATE users SET points=0, chips=10000, wins=0")
                db.execute_query("DELETE FROM game_stats")
        ...
        if cmd in ["ms", "s"]:
            ...
            g_rows = db.execute_query(f"SELECT game_name, wins, earnings FROM game_stats WHERE user_id={db.get_ph()}", (tid,), fetch="all")
        ...
        if cmd in ["gls", "chips"]:
            ...
            msg = f"--- {title} ---\n"
            for i, (un, v) in enumerate(rows or [], 1):
                msg += f"{get_symbol(i, b_type)} {i}. {un[:12]} : {format_k(v)}\n"
            msg += f"--------------\nPage 1 | !nx (30s)"
            with SESSIONS_LOCK:
        ...
        if cmd == "nx":
            ...
            msg = f"--- PAGE {page+1} ---\n"
            for i, (un, v) in enumerate(rows, 1):
                msg += f"- {offset+i}. {un[:12]} : {format_k(v)}\n"
            bot.send_message(room_id, msg); return True
